...
either
\begin{itemize}
  \item A simplicial model category, or
  \item An \(\infty\dash\)category
\end{itemize}
...
\(GW(k)\).
\begin{itemize}
  \item For \(n=0\), this is the rank homomorphism.\\
  \item For \(n=1\) we get the discriminant, which is the determinant of the
linear map associated to the bilinear form obtained after choosing a
basis.
  \item For \(n=2\) this is the Hasse-Witt invariant (see written
notes)
  \item For \(n=3\) this is the Arason invariant
\end{itemize}
...
\Tr(B: V^2 \to L) = V^2 \mapsvia{B} L \mapsvia{\Tr_{W^k}} a
