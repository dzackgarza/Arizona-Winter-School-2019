...
We want to know what the sections are on some scheme $Y$, so we look at its function field and check $K^MW_*$ on it to see which sections are defined over all of $Y$ and not over the generic point.
