...
For example, if $A$ is smooth over $\FF_p$, then $TP(A)$ will look like crystalline cohomology $R\Gamma_\text{crys}(A/\Zp) \definedas \Omega^\wait_{\widetilde{A}/\FF_p}$ where $\widetilde A$ is any smooth algebra over $\Zp$ lifting $A$. A priori, this may depend on choice of lift, so we can't take this as a definition necessarily