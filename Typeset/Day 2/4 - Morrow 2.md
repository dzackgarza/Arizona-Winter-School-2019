...
Since we've written $\tensor$, the structure must be symmetric monoidal (which essentially says that there is some operation that behaves like tensoring and satisfies the properties we know and love). We also want limits and colimits to exist, so we can form 