...
We in fact have $TP(A)[-2] \mapsvia \cong TP(A)$, and the map $u$ becomes $TP(A) \mapsvia{\times p} TP(A)$. This was more or less the extension problem we needed to solve earlier, where we have to check that $u\mapsto p \in TP_0$.