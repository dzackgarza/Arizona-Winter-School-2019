...
**Definition**: The **Nisnevich** coordinates near $x$ are given by $\varphi: U \to \AA^d$ which induces an isomorphism on the function field, so the induced extension of residue fields $k(\phi(p)) \injects k(p)$